...
C2. **IMU cov 保持 0.1, 不要调** (2026-06-11 实验推翻原预测, 见 issue #60). 原以为狗高动态要调到
0.3-0.5 — 同 bag (airy_20260611_144705) 对照实验 acc/gyr_cov ∈ {0.02, 0.1, 0.5} 实测: 0.5 让
植被区 z 蠕变恶化 2 倍 (+6.6→+14.4m), 0.02 与 0.1 无差 (曲线在 0.1 以下饱和). 室外 z 漂移根因是
植被区点到面配准的系统性 z 偏置被 IEKF 当 acc bias 吸收 (偏置速率 0.05-0.17 m/s 落在 b_acc_cov
吸收带, 原理上 cov 调参不可拦截), 不是 IMU 噪声配置问题. 治本方向: 配准前植被点滤除 / z 退化感知
(#60). 采集侧规避: 沿建筑立面走、匀速、不在植被区徘徊、回起点闭环 (街区段 0-150s 三组实验逐点一致,
无此问题).
