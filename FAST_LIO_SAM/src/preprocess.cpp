    ...
    for (uint i = 1; i < plsize; i++) {
        ...
        if ((pl_orig.points[i].line < N_SCANS) &&
            ...
            if (valid_num % point_filter_num == 0) {
                ...
                pl_full[i].curvature =
                    (pl_orig.points[i].timestamp - pl_orig.points[0].timestamp) / 1000000;  // 点的曲率
...
void Preprocess::give_feature(pcl::PointCloud<PointType> &pl, vector<orgtype> &types) {
    ...
    for (uint i = head; i < plsize2; i++) {
        ...
        if (plane_type == 1)  // plane_judge正常退出
        {
            ...
            if (last_state == 1 &&
                last_direct.norm() > 0.1)  // 根据上一状态（局部是平面）和长度（向量模长），决定起始点的类型
            {
    ...
    for (uint i = head + 3; i < plsize2; i++) {
        ...
        for (int j = 0; j < 2; j++)  // 计算当前点与前一点、后一点的向量，判断当前点前后两个方向的edge属性
        {
            ...
            vecs[j] =
                Eigen::Vector3d(pl[i + m].x, pl[i + m].y, pl[i + m].z);  // sensor指向 当前点的前一点或后一点的向量
            vecs[j] = vecs[j] - vec_a;                                   // 当前点指向前一点或后一点的向量

            types[i].angle[j] =
                vec_a.dot(vecs[j]) / vec_a.norm() / vecs[j].norm();  // cos(当前点指向前一点或后一点的向量, ray)
        ...
        types[i].intersect =
            vecs[Prev].dot(vecs[Next]) / vecs[Prev].norm() / vecs[Next].norm();  // 当前点与相邻两点的夹角cos值
