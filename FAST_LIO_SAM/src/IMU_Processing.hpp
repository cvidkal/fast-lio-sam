...
inline M3D GetRotByAlignVector(const V3D& vec, const V3D& vec_ref) {
    V3D vec_normalize = vec.normalized();
    V3D vec_ref_normalize = vec_ref.normalized();
    V3D axis = vec_normalize.cross(vec_ref_normalize);
    ...
    axis.normalize();
    M3D axisSkew;
    axisSkew << 0, -axis.z(), axis.y(), axis.z(), 0, -axis.x(), -axis.y(), axis.x(), 0;
    M3D R = M3D::Identity() + std::sin(angle) * axisSkew + (1 - std::cos(angle)) * axisSkew * axisSkew;
    return R;
}
...
void ImuProcess::UndistortPcl(const MeasureGroup& meas, esekfom::esekf<state_ikfom, 12, input_ikfom>& kf_state,
    ...
    for (auto it_kp = IMUpose.end() - 1; it_kp != IMUpose.begin(); it_kp--)  // imu pose从后向前遍历
    {
        ...
        for (; it_pcl->curvature / double(1000) > head->offset_time;
             it_pcl--)  // head->offset_time为j-1时刻，imu相对于传播起始时刻的时间
        {
            ...
            /***  变换： 终点时刻lidar系 <-- 终点时刻imu（body系） <-- 畸变纠正到global系 <-- 终点时刻imu（body系）<--
             * 终点时刻lidar系 在imu（body）系下进行畸变纠正到w系，可能不太准确 (imu_state.offset_R_L_I * P_i +
             * imu_state.offset_T_L_I) 将i点，按照imu终点时刻的外参，从lidar系转换到终点时刻的imu系下 (R_i * ( P ) +
             * T_i)  畸变纠正，将i点纠正到global系下 (imu_state.rot.conjugate() * ( P - T_e)
             * 将i点旋转到终点时刻的，imu系下 T_i - T_e = T_ei imu_state.offset_R_L_I.conjugate() * （P -
             * imu_state.offset_T_L_I) 变换到终点时刻的lidar系下
             ***/
            V3D P_compensate =
