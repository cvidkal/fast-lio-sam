...
class RosbagIO {
   public:
    RosbagIO() {}
    RosbagIO(std::string bag_file, DatasetType dataset_type = DatasetType::NCLT) : bag_file_(std::move(bag_file)) {}

#ifdef ROS1
    using MsgType = rosbag::MessageInstance;
    using MessageProcessFunction = std::function<void(const MsgType &m)>;

    /// 一些方便直接使用的topics, messages
    using Scan2DHandle = std::function<void(sensor_msgs::LaserScan::ConstPtr)>;

    using PointCloud2Handle = std::function<void(sensor_msgs::PointCloud2::ConstPtr)>;
    using LivoxCloud2Handle = std::function<void(livox_ros_driver::CustomMsg::ConstPtr)>;
    // using FullPointCloudHandle = std::function<bool(FullCloudPtr)>;
    using ImuHandle = std::function<void(sensor_msgs::Imu::ConstPtr)>;
    ...
    RosbagIO &AddPointCloud2Handle(const std::string &topic_name, PointCloud2Handle f) {
        return AddHandle(topic_name, [f, this](const MsgType &m) -> void {
    ...
    RosbagIO &AddImuHandle(const std::string &topic_name, ImuHandle f) {
        return AddHandle(topic_name, [topic_name, f, this](const MsgType &m) -> void {
